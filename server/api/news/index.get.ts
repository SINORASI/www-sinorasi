...
export default defineEventHandler(async (event) => {
  ...
  const { tag, search, limit, offset } = query;
  ...
  if (tag && typeof tag === 'string') {
    filteredNews = filteredNews.filter(news => 
      news.tags.some(t => t.toLowerCase().includes(tag.toLowerCase()))
    );
  }
  ...
  if (search && typeof search === 'string') {
    filteredNews = filteredNews.filter(news => 
      ...
      news.subtitle.toLowerCase().includes(search.toLowerCase())
    );
  }

  // Apply pagination
  const startIndex = offset ? parseInt(offset as string) : 0;
  const endIndex = limit ? startIndex + parseInt(limit as string) : filteredNews.length;
