...
const slug = route.params.organisasi as string

const { data: organization, error } = await useFetch<Organization>(`/api/organizations/${slug}`)
...
useHead({
  title: computed(() => `${organization.value?.name} - Organisasi - SMKN 2 Singosari`),
  meta: [
    {
      name: "description",
      content: computed(() => organization.value?.description),
    },
  ],
});
