...
const { data: allEventsResponse } = await useFetch<{ data: Event[]; total: number }>(
  "/api/events",
  {
    query: {
      limit: 5,
...
const renderedContent = computed(() => {
  ...
  content = content.replace(
    /^### (.*$)/gim,
    '<h3 class="mt-6 mb-3 text-xl font-semibold text-gray-900">$1</h3>',
  );
  content = content.replace(
    /^## (.*$)/gim,
    '<h2 class="mt-8 mb-4 text-2xl font-semibold text-gray-900">$1</h2>',
  );
  content = content.replace(
    /^# (.*$)/gim,
    '<h1 class="mt-8 mb-4 text-3xl font-bold text-gray-900">$1</h1>',
  );
  ...
  content = content.replace(
    /`([^`]+)`/g,
    '<code class="px-2 py-1 text-sm bg-gray-100 rounded">$1</code>',
  );
  ...
  content = content
    ...
    .map((paragraph) => {
      ...
      return (
        '<p class="mb-4 leading-relaxed text-gray-600">' + paragraph.replace(/\n/g, "<br>") + "</p>"
      );
    })
    .join("");
