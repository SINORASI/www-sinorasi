export const majorDatas = {
    rpl: {
        logo: '/images/majorIcon/rpl.webp',
        short: 'RPL',
        nameMajor: 'Rekayasa Perangkat Lunak',
        bgColor: 'bg-orange-600/30 backdrop-blur-2xl border-b-orange-400',
        textColor: 'text-orange-600'
    },
    tkj: {
        logo: '/images/majorIcon/tkj.webp',
        short: 'TKJ',
        nameMajor: 'Teknik Komputer Jaringan',
        bgColor: 'bg-yellow-600/30 backdrop-blur-2xl border-b-yellow-400',
        textColor: 'text-yellow-600'
    },
    dkv: {
        logo: '/images/majorIcon/dkv.webp',
        short: 'DKV',
        nameMajor: 'Desain Komunikasi Visual',
        bgColor: 'bg-blue-600/30 backdrop-blur-2xl border-b-blue-400',
        textColor: 'text-blue-600'
    },
    animasi: {
        logo: '/images/majorIcon/animasi.webp',
        short: 'AN',
        nameMajor: 'Animasi',
        bgColor: 'bg-pink-600/30 backdrop-blur-2xl border-b-pink-400',
        textColor: 'text-pink-600'
    },
    broadcasting: {
        logo: '/images/majorIcon/broadcasting.webp',
        short: 'BC',
        nameMajor: 'Broadcasting',
        bgColor: 'bg-red-600/30 backdrop-blur-2xl border-b-red-400',
        textColor: 'text-red-600'
    },
    tei: {
        logo: '/images/majorIcon/tei.webp',
        short: 'TEI',
        nameMajor: 'Teknik Elektronika Industri',
        bgColor: 'bg-green-600/30 backdrop-blur-2xl border-b-green-400',
        textColor: 'text-green-600'
    },
    mekatronika: {
        logo: '/images/majorIcon/mekatronika.webp',
        short: 'TMT',
        nameMajor: 'Teknik Mekatronika',
        bgColor: 'bg-green-800/30 backdrop-blur-2xl border-b-green-400',
        textColor: 'text-green-800'
    },
    tav: {
        logo: '/images/majorIcon/tav.webp',
        short: 'TAV',
        nameMajor: 'Teknik Audio Video',
        bgColor: 'bg-green-400/30 backdrop-blur-2xl border-b-green-400',
        textColor: 'text-green-400'
    },
}