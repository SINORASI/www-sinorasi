# Example: Updated Component Using API Routes

## Before (using data.ts)
...
const { data: allNewsResponse } = await useFetch('/api/news', {
  query: {
    limit: 5  // Get 5 to ensure we have 4 after filtering
  }
})

const relatedNews = computed(() => {
  if (!news.value || !allNewsResponse.value?.data) return []
  return allNewsResponse.value.data.filter(n => n.id !== news.value!.id).slice(0, 4)
})
</script>
```

## Complete Updated File: `app/pages/berita/[slug].vue`
    ...
    <div v-if="news" class="max-w-4xl mx-auto">
      ...
      <section id="berita-lainnya" class="mt-12 pt-8 border-t border-gray-200">
        ...
        <div class="flex space-x-6 overflow-x-auto pb-4 scrollbar-hide">
          <div v-for="related in relatedNews" :key="related.id" class="flex-shrink-0 w-64">
...
1. **Removed import**: No more `import { newsData } from '~/datas/data'`
2. **Added API fetch**: `await useFetch(\`/api/news/\${slug}\`)`
3. **Updated related news**: Now fetches from API with limit parameter
4. **Type safety maintained**: Still using `type { News }` from models
5. **Error handling**: Can use the `error` object from useFetch for better UX

## Benefits

- ✅ Data fetching happens on server (better SEO)
- ✅ Can easily switch to external API later
